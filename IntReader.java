...
public interface IntReader {
    ...
    /**
     * Obtains the next {@code int} from the input stream. Does not validate
     * {@code int} values, but will loop if the user's input cannot be parsed
     * as an {@code int}.
     *
     * @param prompt The prompt to show the user
     * @return the move
     ...
     * @throws NoSuchElementException if the input stream ends
     */
    default int nextInt(String prompt) throws IOException {
        return nextInt(prompt, i -> null);
    }
